...
def find_format(val):
    ...
    if any([val.strip().lower().find(f) == len(val) - len(f) for f in SETTINGS.file_formats.photo]):
        return 'photo'

    if any([val.strip().lower().find(f) == len(val) - len(f) for f in SETTINGS.file_formats.audio]):
        return 'audio'

    if any([val.strip().lower().find(f) == len(val) - len(f) for f in SETTINGS.file_formats.video]):
        return 'video'
